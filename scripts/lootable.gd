...
func _process(delta):
	if lootable and get_rect().has_point(to_local(get_global_mouse_position())) and owner.get_parent().player.global_position.distance_to(get_global_mouse_position()) <= pick_up_dis:
